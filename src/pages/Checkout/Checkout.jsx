...
const Checkout = ({ debouncedApiCall, setStep, rows, setOrderSuccess, orderSuccess, shopes }) => {
   ...
   async function updateCartData(){
    ...
    return (
        <Grid container >
            <Grid item md={12} sxm={12} xs={12} sx={{ display: { xs: "block", sm: "block", md: "none" } }} style={{ paddingLeft: "15px", height: "70px" }} >
                <br />
                <Typography variant='h3'>Checkout</Typography>
                <br />
            </Grid>
            <Grid item container md={8} sm={12} sx={{ maxHeight: { xs: "calc(100vh - 148px)", sm: "100vh" }, paddingTop: "10px", background: { xs: "#f8f8f8" } }} xs={12} className='checkoutMain flex' >

                <Grid item container md={10} sm={11} xs={11}  >

                    <Grid item md={12} sm={12} sx={{ display: { xs: "none", sm: "none", md: "block" } }} onClick={() => navigate("/")} className='checkoutLogo' ></Grid>
                    <Grid item md={12} sxm={12} xs={12} sx={{ display: { xs: "none", sm: "none", md: "block" } }} >
                        <br />
                        <Typography variant='h3'>Checkout</Typography>
                        <br />
                    </Grid>
                    <Grid item md={12} sm={12} xs={12} sx={{ paddingBottom: { xs: "10px" } }} >
                    ...
                    <Grid container md={12} sm={12} style={{ display: "flex", alignItems: "center", flexDirection: "row" }} >
                        <br />
                        <Grid item md={3.5} sx={{ display: { xs: "none", sm: "block" } }} >
                            <Typography variant='p' style={{ color: "grey" }} ><span style={{ color: "#e46e39" }} >Enter Reciever Name:</span></Typography>
                        </Grid>
                        <Grid item md={4} sx={{ marginTop: { xs: "10px", sm: "0px" } }} >
                            <Input type="text" placeholder="Enter Reciever Name" value={rName} setValue={setRName} border={true} />
                        </Grid>
                    </Grid>
                    <Grid item md={12} sm={12} sx={{ paddingBottom: { xs: "10px" } }} >
                        <br />
                    ...
                    <Grid container xs={12} >
                        <Grid item md={12} sm={12} s={12} >
                            <br />
                            <Upload {...props}>
                                <Typography variant='p'>If you have any PO click here to <Typography variant='p' sx={{ fontWeight: "bold", textDecoration: "underline" }} >ATTACH</Typography> </Typography>
                            </Upload>

                        </Grid>

                    </Grid>
                    <Grid item container md={12} sm={12} className='checkoutPageCheckoutButtons' >
                        <button onClick={() => navigate('/quickOrder/1')} className='checkoutButtonBack flex' ><KeyboardArrowLeftIcon />Back</button>
                        <button className='checkoutButton flex' style={loadingState ? { backgroundColor: "transparent", border: "1px solid #e46e39" } : {}} onClick={() => placeOrder()} >
                            {!loadingState ?
                                <b> Place Order</b>
                                : <CircularProgress style={{ color: "#e46e39", width: "25px", height: "25px" }} />
                            }
                        </button>
                    </Grid>
                </Grid>
                                ...
                                rows.map((item, index) => {
                                    if (item.LottypeCode.value && item.ShadeCode.label && item.LottypeCode.HsCode && item.OrderQty && item.selectedYardage.label) {
                                        return (<Grid container className='yourCartRow' >
                                            ...
                                            <Grid item md={4} sm={4} >
                                                <QuantityCheckout debouncedApiCall={debouncedApiCall} item={item} rows={rows} index={index} />
                                            </Grid>
                                            {/* <Grid item md={4} style={{ textAlign: "center" }} >{item.OrderQty}x</Grid> */}
                                            <Grid item container md={1} sm={1} >
            ...
            <PopupAlert open={showPopup} message={message1} severty={severty} onClose={handleClosePopup} />
        </Grid>
